...
char *readcsv(const char *path) {
    ...
    while (feof(pointer_file) == 0) {
        char character = fgetc(pointer_file);
        if (character == '\n') {
            rows++;
        } else if (rows < 2 && character == 44) {
            columns++;
        }
    }
    ...
    int columnas = 0;
    dataType *arrays;
    arrays = (dataType *) malloc(rows * columns * sizeof (dataType));
    while (i < rows) {
        columnas = 0;
        fgets(space, 10000, pointer_file);
        almacenamiento = strtok(space, ",");
        while (almacenamiento != NULL) {
            strcpy(*(arrays + i * columns + columnas), almacenamiento);
